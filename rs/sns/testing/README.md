...
2) Adjust init YAML file (you will need `yq` to be installed to do this):
   ```
   yq -i -Y ".dapp_canisters |= [\""$(dfx canister --network http://127.0.0.1:8080 id test)"\"]" ../cli/test_sns_init_v2.yaml
   yq -i -Y ".Distribution.Neurons[0].principal |= \""$(dfx identity get-principal --identity sns-testing)"\"" ../cli/test_sns_init_v2.yaml
   yq -i -Y ".Swap.start_time |= null" ../cli/test_sns_init_v2.yaml
   ```
3) Propose to create the new SNS:
   ```
   pushd ../cli
   bazel run //rs/sns/cli:sns -- --identity sns-testing propose --network http://127.0.0.1:8080 --neuron-id 449479075714955186 $PWD/test_sns_init_v2.yaml
   popd
   ```
</details>
